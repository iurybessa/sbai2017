...
\commentib{Voc\^e sabe a origem daquele \'indice no topoda primeira p\'agina? Isso n\~ao pode ficar a\'i!}
...
\commentib{Eu comentei algumas das refs que estavam dando pau na hora de compilar. Revise-as, eleas devem estar usando algum s\'imbolo estranho. Al\'em disso, evite citar sites. Se n\~ao puder evitar, existe o campo de bibtex pr\'oprio para eles.}
...
The remaining of this work is organized as follows:\commentib{COMPLETAR. DIga o que vir\'a em cada se\c{c}\~ao. Aconselho que voc\^e crie um label para cada se\c{c}\~ao para que possa referenciar (veja como fi\ na introdu\c{c}\~ao}).


\section{Background}

\subsection{Mission Planning}
Mission planning for UAVs can be defined as the process of location planning (waypoints) to be visited and the actions of the vehicle (loading/unloading, filming/photos, capturing information), usually within a certain range of time \commentib{Essa defini\c{c}\~ao est\'a um pouco sem sentido. Procure uma defini\c{c}\~ao na literatura e insira a cita\c{c}\~ao da sua fonte.}. Functionally, mission planning lies above the trajectory planning process, where the mission planner generates a desired mission plan, and then the trajectory planner generates the flight plan (trajectories) between the waypoints. \commentib{Acho que voc\^e pode reformular esse par\'agrafo, defina primeiramente o que \'e uma miss\~ao, depois d\^e essa defini\c{c}\~ao de planejamento (como vc fez), e finalmente diga o que \'e o planejador. Deixe claro que o planejador \'e um software.}
...
\commentib{Os par\'agrafos anteriores n\~ao fazem parte de um background sobre mission planning, eles s\~ao trabalhos relacionados. Crie uma se\c{c}\~ao s\'o para isso.}
...
\begin{figure}[H]
	...
	\caption{Some Optimization Problems.\commentib{N\~ao \'e poss\'ivel ler essa figura.} \label{fig:problemasDeOtimizacao}}
	\end{figure}
