...
\begin{itemize}
\item an evaluation methodology for UAV mission planners algorithms, \commentib{fale um pouco mais};
...
\commentib{Fale um pouco mais aqui sobre o framework desenvolvido para comando do UAV }
...
Mission command is a command created to execute a task such as go to one location to another location, get a package using a robot gripper, land the drone, etc. The list of commands is shown below: \commentib{Isso tem que aparecer na se\c{c}\~ao anterior.}
...
\item \texttt{Land}: command to make the UAV landing.
\end{itemize}
...
C_{max}=makespan
\end{equation}
\commentib{o que \'e isso? Esse modelo para $C_{max}$ n\~ao est\'a claro}
...
\end{equation}
\commentib{Isso t\'a errado. A fun\c{c}\~ao objetivo n\~ao pode ser limitada inferiormente conforme a eq anterior.}
...
In order to clarify the evaluation methodology efficiency, two mission planner algorithms are evaluated using the proposed metric, and the evaluation result is compared with practical execution time of algorithms that are implemented in a simulation tool \commentib{qual o simulador...? fale mais} and in a real 3DR IRIS+ UAV system. The mission planning algorithm and the command and control framework is executed in a \commentib{... especifique melhor as caracter\'isticas do computador usado, e diga o papel do dronekit API}
...
In this work, a common application of intra logistics performed by a VANT was developed, as well as the implementation of two mission planning strategies, as well as the cost evaluation of the same, compared to the optimal cost obtained by the CPLEX optimizer. \commentib{Reescreva aqui as contribui\c{c}\~oes relevantes do trabalho... fale da metodologia de avalia\c{c}\~ao e ddo framework de controle desenvolvido.}
