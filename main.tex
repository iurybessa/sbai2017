...
\begin{itemize}
\item an evaluation methodology for UAV mission planners algorithms,  \commentib{fale um pouco mais}\commentib{nao sei oq mais falar rs};
...
In this section, we first investigate the platform used in this work, verifying the hardware characteristics and how to control it by software. The core hardware of the UAV IRIS+ is the Pixhawk and we can control it using a Python library - dronekit that uses MAVLink\footnote{MAVLink or Micro Air Vehicle Link is a protocol for communicating with small unmanned vehicle. It is designed as a header-only message marshalling library} protocol. 

Secondly, we integrate a robot gripper to the UAV to take and leave packages during the mission. We have connected a servo motor to the Pixhawk by one of the PWM (pulse width modulation) outputs. After that, we created movement functions to make the UAV movement, such as go to a location, and control it using some resources of the dronekit api. 

\begin{figure}[H]
	\centering
	\includegraphics[width=0.45\textwidth]{movement.eps}
	\caption{UAV Movement Systems.\label{fig:movement}}
\end{figure}

The movement functions are shown below:
...
As we can see in the Figure~\ref{fig:sysArch}, there is a mission planner that generates the mission through the existing inputs in the warehouse and the client's order. After mission generation (using some algorithmic strategy), the UAV receives the mission and executes it. In the Section~\ref{sec:method} there is a more comprehensive explanation of the terms used in this current section. 
...
The variable $x_{ij}$ is a binary decision variable which takes the value $1$ if the task $j$ is running on the machine $i$, and $0$ otherwise. The variable $C_ {max}$ is the variable that we want to optimize.
...
C_{max}=makespan
\end{equation}
\commentib{o que \'e isso? Esse modelo para $C_{max}$ n\~ao est\'a claro}\commentib{ja tem uma explicacao do que eh makespan anteriormente.}
...
$C_{max}$ is the variable to be minimized (optimized).
%\begin{equation}
%\label{eq:cmax}
...
\end{equation}
\commentib{Isso t\'a errado. A fun\c{c}\~ao objetivo n\~ao pode ser limitada inferiormente conforme a eq anterior.}\commentib{eh uma restricao apenas iury. nao ta errado. a funcao objetivo eh o $C_{max}$ (ja eh outra coisa). Organizei exatamente como o pessoal de otimizacao usa nas literaturas.} 

The Eq.~\eqref{eq:execTime} represents the sum of the the duration time $p_j$ of each travel from one place another in the case study explained in Section~\ref{sec:ec}, considering the production time $s_j$ in each production line. The $x_{ij}$ shows which product is being produced in each production line. The left hand side of the Eq.~\eqref{eq:execTime} needs to be smaller than the right hand side of the Eq.~\eqref{eq:execTime}.
\end{itemize}
...
In order to clarify the evaluation methodology efficiency, two mission planner algorithms are evaluated using the proposed metric, and the evaluation result is compared with practical execution time of algorithms that are implemented in the simulation tool SITL\footnote{Simulator that allows executing a Plane, Copter or Rover without the need of a hardware)} and in a real UAV system (3DR IRIS+). The mission planning algorithms were executed in a computer running Linux Mint, core i7 processor and 8GB of RAM. In order to control the UAV, we run the control program, which uses the dronekit api as an interface between a high level program language (Python) and the protocol that the UAV understand (MAVLink), in the same computer where there is a radio module connected via usb communicating with the UAV radio module.


\subsection{Mission Planners}

In this work, we considered that mission planner is a software that generates a production mission given the warehouse and customer order. This program generates a \texttt{.mission} extension file containing a set of mission commands, as shown in the Section~\ref{missao}. Two examples of planners are presented in this work and will be employed to demonstrate the cost evaluation methodology.

\subsubsection{Planner A}

In the Algorithm~\ref{alg:planA}, we show a strategy to solve the mission planning problem and we denoted as Planner A. In this Algorithm, it's produced all the X product in sequence, bringing the inputs in sequence as well (A,B and C) and then it's produced all the Y products in this order.
...
The following is data about the mission being executed in the simulator SITL. The table below shows the performances of both schedulers relative to flight time.
...
\begin{figure}[H]
	...
	In Table~\ref{table:plannersReal}, it can be seen that the mission time executed by planner B in real environment also shown a shorter time compared to planner A in the five tests:
...
\begin{figure}[H]
	...
	As presented in the section, planner B was faster than planner A, relative to execution time. This can also be verified in real environment, as shown in tables \ref{table:plannersSimulado} and \ref{table:plannersReal} of mission execution time in simulated and real environment.
...
Further works includes the use of computational vision for the recognition of inputs, and to improve the modelling of the optimization problem for better results in cost evaluation.
