...
The Mission Planner reads the warehouse inputs and client order and produces a \texttt{.mission} file, which contains the list of mission commands needed for producing the required client order. This \texttt{.mission} file is used by an UAV Control Program to control the UAV and to produce the low-level movement commands (cf. Definition~\ref{def:missioncommand}).
...
\item \texttt{Wait}: command to make an UAV to hover (wait);
\item \texttt{Land}: command to make an UAV to land.
\end{itemize}
