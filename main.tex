...
\commentib{O que mais eu poderia falar?}
...
Mission command is a command created to execute a task such as go to one location to another location, get a package using a robot gripper, land the drone, etc. The list of commands is shown below:
...
In order to succeed the experimentations, we have used the IRIS+ UAV, dronekit API and a computer with windows 10 to run the program that sends the control commands to the UAV. The experimentation was made in a grass field at university  in order to have a clear area of operation.

The objectives of the experimentations were the measurements of execution time of the mission using different planning solutions.

We also have run the same experiments in a simulated environment. We have used the SITL\footnote{The SITL (software in the loop) simulator allows you to run Plane, Copter or Rover without any hardware.} simulator to run the experiments sing the same variables of experiment.
