...
In this section, we first investigate the UAV platform used (3DR IRIS+) in this work, verifying the hardware characteristics and the control framework developed for intralogistics missions. The core hardware of the UAV IRIS+ is the Pixhawk and we can control it using a Python library (dronekit) that uses Micro Air Vehicle Link (MAVLink) protocol \commentib{REF para MAVLink}. MAVLink is a protocol for communicating with small unmanned vehicle. It is designed as a header-only message marshalling library. 
