...
\begin{itemize}
\item an evaluation methodology for UAV mission planners algorithms;
...
The movement functions illustrated above are in a file, of extension .mission, that the UAV reads and executes that commands. The file is generated by a mission planner program.
...
\sum_{i=1}^{M}{\sum_{j=1}^{N}{
 (p_j+s_j)x_{ij}}}\leq \hat{C}_{max}
\end{equation}

where $\hat{C}_{max}$ is a variable used to set a ceiling value to $C_{max}$ during the implementation.
...
\begin{equation}
	...
	\begin{array}{ccc}
		\min & C_{max},  \\
  \\
		\textrm{ subject to } & \sum_{i=1}^{M}{\sum_{j=1}^{N}{x_{ij}}}=1, \\
		& \sum_{i=1}^{M}{\sum_{j=1}^{N}{
